...
2. [Requirements](#requirements) -> **AnsiColors.hpp only depents on the standard C++ library; Windows hosts must first check whether they're running a compatible Windows version.** <br> *Note: **`Almost all modern Windows systems are supported`**; unfortunately, **`this header doesn't work on legacy Windows systems.`***
